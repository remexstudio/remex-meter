...
The order is the fresh-install order; a user's saved order is never overwritten. Tool ids and data sources are in `docs/PROVIDERS.md`. The inherited upstream supports many more tools; they stay wired and selectable in Settings but are off on a fresh install.
